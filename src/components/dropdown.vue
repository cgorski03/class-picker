<template>
  <div class="course-dropdown">
    <div class="course-header" @click="toggleDropdown">
      <span class="arrow"
        >{{ showDropdown ? "▼" : "▶" }} {{ course.classTitle }}</span
      >
    </div>
    ...
    <div
      ...
      <p>
        <strong
          >{{ course.classSubj }} {{ course.classCANum }}
          {{ course.classTitle }}</strong
        >
        {{ course.teacher }}
      </p>
      <p>
        Meeting Time: {{ course.startTime }} - {{ course.endTime }} on
        {{ convertToFullDays(course.daysMeet) }}
      </p>
      <div v-if="!isStudent">
        <p><strong>Student Roster: </strong></p>
        <p
          v-for="student in getStudentList(course.studentList)"
          :key="'student'">
          {{ student }}
        </p>
      </div>
